...
class CampaignSortingConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if text_data_json['type'] == 'vote_update':
            ...
            await self.process_vote_update(text_data_json)

    async def process_vote_update(self, data):
        # Extract necessary data from the received JSON
        candidate_id = data['candidate_id']
        new_votes = data['votes']

        # Update the vote count in the database
        await self.update_vote_count(candidate_id, new_votes)

        # Notify the group about the vote update
        await self.channel_layer.group_send(
            self.room_group_name,
            {
                'type': 'send_vote_update',
                'message': f'Updated votes for candidate {candidate_id} to {new_votes}'
            }
        )

    async def send_vote_update(self, event):
        # Send a message with the updated vote count
    ...
    @sync_to_async
    def update_vote_count(self, candidate_id, new_votes):
        # Logic to update vote count in the database
        # Assuming CampaignSorting model has a foreign key to a Candidate model
        sorting = CampaignSorting.objects.get(candidate_id=candidate_id)
        sorting.votes = new_votes
        sorting.save()
