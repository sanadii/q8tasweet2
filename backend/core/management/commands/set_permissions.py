    ...
    for model, permissions in PERMISSIONS.items():
        ...
        try:
            ...
            for permission_codename, groups in permissions.items():
                ...
                if created:
                    print(f"Created new permission: {permission_codename} for model {model}")

                # Assign the permission to the groups
                for group_codename in groups:
                    try:
                        group, created = Group.objects.get_or_create(codename=group_codename)
                        if created:
                            print(f"Created new group: {group_codename}")
                        group.permissions.add(permission)
                    except IntegrityError:
                        print(f"IntegrityError: Skipping group {group_codename} as it already exists.")
                        continue
