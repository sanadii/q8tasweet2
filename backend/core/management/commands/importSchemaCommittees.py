    ...
    def handle(self, *args, **options):
        ...
        for index, row in df.iterrows():
            try:
                ...
                committee_site_instance = CommitteeSite.objects.get(
                    id=row["committee_site"]
                )
                ...
                defaults = {
                    ...
                    if col != "id" and col != "committee_site"
                }
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Committee with ID {row['committee_site']} does not exist."
                    )
                )
            except Exception as e:
                self.stdout.write(
                    self.style.WARNING(
                        f"Error occurred while importing or updating committees: {e}"
                    )
                )
        ...
        self.stdout.write(
            self.style.SUCCESS(f"Updated: {updated_count} committees due to updates")
        )
