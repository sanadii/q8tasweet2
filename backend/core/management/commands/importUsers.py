...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        file_path = "core/management/startup/q8tasweet.xlsx"
        work_sheet = "users"
        required_data = [
            "id",
            "first_name",
        ...
        for index, row in df.iterrows():
            try:
                    ...
                    if isinstance(field, ForeignKey):
                        ...
                        if pd.isna(field_value) or field_value == "":
                            ...
                            try:
                                ...
                                user_instance = User.objects.get(id=field_value)
                                ...
                                self.stdout.write(
                                    self.style.WARNING(
                                        f"User with ID {field_value} does not exist."
                                    )
                                )
                                defaults[field.name] = (
                                    None  # Assign None if user doesn't exist
                                )
                ...
                self.stdout.write(
                    self.style.WARNING(
                        f"Error occurred while importing or updating user: {e}"
                    )
                )
