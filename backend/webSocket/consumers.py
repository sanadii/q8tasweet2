...
class GlobalConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        await self.accept()
        welcome_message = json.dumps({'message': 'Welcome to the Global channel'})
        await self.send(welcome_message)

    async def disconnect(self, close_code):
        pass  # Handle any cleanup here
    ...
    async def receive(self, text_data):
        ...
        if message_type in ['Public', 'Private']:
            response_message = json.dumps({'response': f'Received {message_type} message: {message}'})
            await self.send(response_message)
        else:
            error_message = json.dumps({'error': f'Received unknown type: {message_type}'})
