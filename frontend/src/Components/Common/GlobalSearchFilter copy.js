...
import { StatusOptions, PriorityOptions } from "../../common/data/elections";
...
const ElectionListGlobalFilter = () => {
    ...
    const { electionList } = useSelector(state => state.Elections);

    const [filteredElectionList, setFilteredElectionList] = useState(electionList);
    ...
    const electionStatus = (e) => {
        ...
        if (e) {
            let filteredList = electionList.filter((item) => item.status === e);
            console.log("Filtered list when e is truthy:", filteredList);
            setFilteredElectionList(filteredList);
        } else {
            let filteredList = electionList.filter((item) => item.status !== e);
            console.log("Filtered list when e is falsy:", filteredList);
            setFilteredElectionList(filteredList);
        }
    };
