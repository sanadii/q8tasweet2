...
export const ResultInputField = ({ candidateId, committeeId, value, onChange }) => {
  ...
  const handleBlur = () => {
    ...
    if (committeeId) {
      onChange(candidateId, committeeId, localVotes);
    } else {
  ...
  const handleChange = (e) => {
    ...
    // Ensure we only set state for numeric values or empty string (for backspace)
    if (!newValue || newValue.match(/^\d+$/)) {
      setLocalVotes(newValue);
    }
  };

  return (
    <input
      key={committeeId ? `${candidateId}-${committeeId}` : candidateId}
      type="number"
      maxLength="5"
      pattern="\d*"
      style={{ width: "5em" }}
